...
class DiffInfo(buildstep.ShellMixin, buildstep.BuildStep):
    name = 'diffinfo'
    description = 'running diffinfo'
    descriptionDone = 'diffinfo'
    ...
    def __init__(self, compareToRef='master', dataName='diffinfo-master', **kwargs):
        try:
            ...
            config.error('unidiff package must be installed in order to use DiffInfo')
    ...
    def run(self):
        ...
        self.setProperty('diffinfo-merge-base-commit', commit, 'DiffInfo')

        self.addLogObserver('stdio-diff', self._observer)

        command = ['git', 'diff', '-U0', commit, 'HEAD']
        ...
        yield self.setBuildData(self._data_name, data, 'DiffInfo')
